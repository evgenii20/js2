...
<!--      <Cart @get-cart="getCart" :cartProducts="cartProducts" :isVisibleCart="isVisibleCart"/>-->
<!--      :get-cart -> props -->
      <Cart :postJson="postJson" :get-cart="getCart" :cartProducts="cartProducts" :isVisibleCart="isVisibleCart"/>
    </main>
  </div>
...
export default {
  ...
  // Для обращения через API
  // mount - вызывается в момент когда наша компонента встраивается
  // в ДОМ
  mounted() {
  ...
  methods: {
    ...
    addToCart(item) {
      ...
      this.postJson(`${API}/addToCart`, item)
          .then(() => this.getCart())
      //  -------
    ...
    getProducts() {
      // this.getJson(`${API}/catalogData.json`) /database/catalog.json
      this.getJson(`${API}/catalogData`)
          .then((data) => {
            this.products = data;
            this.filtered = data;
